...
export class EndComponent extends ComponentBaseEntity {
    name: string;
    eType: string = "End";
    gs: GameState;
    constructor(gs: GameState, pos: IVec, onTouch?: () => void) {
        const { stage } = gs;
        super(stage, []);
        this.gs = gs;
        const position = new StaticPositionComponent(pos);
        const renderer = new ImgRenderComponent(gs.images["static"].endBlock);
        const box = new BoxColliderComponent([32, 32], e => {
            if (e.eType === "Player") {
                onTouch?.();
            }
        });
...
const generateMap = (gs: GameState, scene: Scene, mp: null | Map = null, end: (() => void) | null = null) => {
    // Generate map
    const step = ["4.", "1|", "10."];
    ...
    for (let i = 0; i < map.length; i++) {
        ...
        if (tile === "e") {
            scene.addEntity(new EndComponent(gs, [i * 32, v - 64], end));
        }
    ...
    return new Scene(
        async (gs: GameState, scene): Promise<{ gs: GameState; scene: Scene; win: boolean }> =>
            new Promise(resolve => {
                ...
                let map = generateMap(gs, scene, null, !!infinite ? null : () => resolve({ gs, scene, win: true }));
                ...
                gl.onUpdate(delta => {
                    ...
                    if (infinite && Math.round(x / 32) >= map.length - 100) {
